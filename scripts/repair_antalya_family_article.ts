...
async function generateImageVertex(prompt: string, filename: string, retries = 3) {
  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      // Rate Limit Delay
      if (attempt > 1) await sleep(20000); // 20s wait between retries
...
async function repair() {
  const timestamp = 1767901516802; // Reuse previous run timestamp
  console.log("🛠️ Starting Repair for Antalya Family Article...");

  // SAFTEY PROMPTS (No kids explicitly to avoid filter blocks)
  const imagesToGenerate = [
    {
      placeholder: '<!-- IMAGE_COVER_PLACEHOLDER -->',
      filename: `antalya-family-cover-uk-${timestamp}.jpg`,
      prompt: "A candid iPhone photo of tourists walking near the Antalya cliffs/seafront. Bright authentic daylight. Blue sky, Mediterranean sea. 'Real holiday' vibe. slightly imperfect composition. No AI gloss. 35mm. Realistic colors."
    },
    {
      placeholder: '<!-- IMAGE_LARA_PLACEHOLDER -->',
      filename: `antalya-lara-family-beach-${timestamp}.jpg`,
      prompt: "A realistic beach scene in Lara, Antalya. Sandy beach, some colorful towels and umbrellas. People relaxing in the distance. Bright sunny day. Authentic travel photo style. Low angle shot."
    },
    {
    ...
    },
    {
      ...
      prompt: "Evening twilight in Antalya Old Town (Kaleici). Warm street lights, stone paved narrow street. People walking in the distance. Cozy atmosphere. Authentic / raw photo style. No bokeh overkill."
    }
  ];
  ...
  for (const item of imagesToGenerate) {
    if (!item.placeholder.includes('COVER')) await sleep(5000); // 5s buffer
    ...
    if (publicUrl) {
      ...
      console.warn("⚠️ Image generation failed AGAIN for:", item.filename);
      // Remove placeholder
      finalContent = finalContent.replace(item.placeholder, '');
    }
  }
  ...
  const { error } = await supabase.from('articles').upsert({
    ...
    title: { en: ARTICLE_DATA.title, tr: "Antalya Aile Tatili Rehberi (TR Pasif)" },
    ...
    cover_image_url: coverImageUrl, // Update with correct cover URL (even if it existed, we need it here)
  ...
  if (error) {
    ...
    console.log("✅ Article Repaired and Updated Successfully!");
  }
}
